...
public interface TalentDemandDao {
     ...
     * @return
     */
    @Insert("insert into talent_demand (user_name,company_name,province,city,district,title,content,engaged_industry,engaged_industry_detail,industry_experience,technology_scope,research_direction,demands_type,people_num,remuneration,salary,contact,phone_num,create_time)" +
            " values (#{userName},#{companyName},#{province},#{city},#{district},#{title},#{content},#{engagedIndustry},#{engagedIndustryDetail},#{industryExperience},#{technologyScope},#{researchDirection},#{demandsType},#{peopleNum},#{remuneration},#{salary},#{contact},#{phoneNum},now())")
